...
The folder `example_outputs/` contains output files generated by running the tool on a small test dataset (`test_data/`) included in this repository.
Included example output files:
- `example_outputs/example_barcode_count_summary_unfiltered.csv`
- `example_outputs/example_barcode_count_summary_validated.csv`
- `example_outputs/example_context_distribution_percent.csv`
- `example_outputs/example_correction_summary.csv`
- `example_outputs/example_barcode_count_distribution.png`
- `example_outputs/example_correction_summary.png`

These were generated from a lightweight synthetic dataset designed for testing. You can run them yourself using:
```bash
python analyze_barcodes.py \
  --r1 test_data/example_R1.fastq \
  --r2 test_data/example_R2.fastq \
  --min-pct 40 \
  --context CTA \
  --corrected-context CCA \
  --anchor1 CGTAC \
